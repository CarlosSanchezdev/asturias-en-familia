...
Cada caso de uso incluye: actores, precondiciones, flujo principal,
flujos alternativos, postcondiciones y el test asociado.
...
**Actor:** Visitante
**Precondición:** Hay al menos una actividad activa en la BD
...
- En el paso 2, si no hay actividades, el mapa se muestra vacío
  con el mensaje "No hay actividades disponibles todavía"
...
**Actor:** Visitante
**Precondición:** Hay actividades de distintas zonas en la BD
...
- Si no hay actividades en la zona seleccionada, el mapa muestra
  el estado vacío y el contador indica "0 actividades"
...
**Actor:** Visitante
**Precondición:** Hay al menos una actividad visible en el mapa

### Flujo principal
1. El visitante hace clic en un marcador del mapa
2. El sistema carga el detalle de la actividad (`GET /api/activities/:id`)
3. En móvil: aparece un panel deslizante desde abajo
   En escritorio: aparece un panel lateral derecho
4. El panel muestra: nombre, descripción, categoría, precio, accesibilidad,
   idiomas e imágenes
...
- Si la petición falla, el panel muestra un mensaje de error
...
**Precondición:** El admin ha iniciado sesión; existe al menos una categoría

### Flujo principal
1. El admin navega a `/admin/actividades/nueva`
2. Rellena el formulario: nombre, descripción, categoría, zona, coordenadas,
   precio, accesibilidad, idiomas
...
4. Si inicia sesión como `visitor`, al intentar acceder a `/admin`
   lo redirige a `/` con aviso
...
## CU-10 — Registro de nuevo usuario

**Actor:** Visitante

### Flujo principal
1. El visitante navega a `/auth/register`
2. Introduce email y contraseña válidos
3. El sistema crea el usuario con rol `visitor`
4. El sistema inicia sesión automáticamente (devuelve tokens)
5. El sistema redirige al mapa (`/`)

### Flujo alternativo A — Email ya registrado
- El backend responde 409
- El formulario muestra: "Este email ya está registrado"

### Flujo alternativo B — Contraseña débil
- Validación en cliente: muestra los requisitos no cumplidos
- No envía la petición

### Tests asociados
- `[Jest]` `POST /api/auth/register` con email nuevo → 201 + tokens
- `[Jest]` `POST /api/auth/register` con email duplicado → 409
- `[Jest]` `POST /api/auth/register` con contraseña sin mayúscula → 400
- `[Jest]` La contraseña no se devuelve en la respuesta
- `[Jest]` El hash de bcrypt se guarda (no el texto plano)
